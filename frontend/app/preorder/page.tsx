...
import { Trash2, Calendar, ArrowLeft, CheckCircle2 } from 'lucide-react';
import Link from 'next/link';

export default function PreorderPage() {
  const { preorderCart, removeFromPreorder, updatePreorderQuantity, getPreorderTotal, clearPreorderCart } = usePreorder();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (preorderCart.length === 0) {
      setErrorMsg('Đơn đặt trước của bạn đang trống.');
      return;
    }
    ...
    try {
      ...
      const orderPayload = {
        ...
        total_price: getPreorderTotal()
      };

      await fetchAPI('/api/preorders', {
        method: 'POST',
      ...
      setErrorMsg(err.message || 'Có lỗi xảy ra khi gửi đơn đặt trước. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="container" style={{ padding: '60px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div className="card" style={{
          ...
          <p style={{ color: 'var(--text-secondary)', marginBottom: '25px', fontSize: '0.95rem' }}>
            Cảm ơn bạn đã tin tưởng BANMAYFILM! Chúng tôi đã nhận được yêu cầu Pre-order của bạn.
            {paymentMethod === 'Chuyển khoản' && (
              <strong style={{ display: 'block', marginTop: '10px', color: 'var(--danger)' }}>
                Vui lòng gửi ảnh chụp màn hình chuyển khoản (50% giá trị đơn hàng) vào tài khoản Instagram @banmayfilm để xác nhận!
              </strong>
            )}
          </p>
    ...
    <div className="container" style={{ margin: '40px auto' }}>
      ...
      {preorderCart.length === 0 ? (
        <div style={{
          ...
          <Calendar size={48} style={{ margin: '0 auto 15px', color: 'var(--text-secondary)' }} />
          <p style={{ fontSize: '1.1rem', fontWeight: 500, marginBottom: '20px' }}>Chưa có sản phẩm Pre-order nào.</p>
            ...
            <form onSubmit={handleSubmit} className="card" style={{ padding: '24px', backgroundColor: 'var(--bg-secondary)' }}>
              ...
              {paymentMethod === 'Chuyển khoản' && (
                <div style={{
                  ...
                  Thông tin chuyển khoản (Vui lòng cọc 50%): <br />
              ...
              >
                {loading ? 'Đang xử lý...' : 'Xác nhận Pre-order'}
              </button>
            </form>
          </div>
            ...
            <div className="card" style={{ padding: '24px', position: 'sticky', top: '100px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px', marginBottom: '25px', maxHeight: '400px', overflowY: 'auto', paddingRight: '5px' }}>
                {preorderCart.map((item) => (
                  <div 
                    key={item.product_id + item.name} // Unique key
                    style={{
                      display: 'flex',
                      gap: '12px',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', flexGrow: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '6px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          <button
                            type="button"
                            onClick={() => updatePreorderQuantity(item.product_id, item.quantity - 1)}
                    ...
                    <button
                      type="button"
                      onClick={() => removeFromPreorder(item.product_id)}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: 'var(--text-secondary)',
              ...
              <div style={{
                ...
                <span style={{ fontSize: '1rem', fontWeight: 700 }}>Tổng tiền cọc (50%):</span>
                <span style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--text-primary)' }}>
                  {formatVND(getPreorderTotal())}
                </span>
              </div>
            </div>
          </div>
